...
$(document).ready(function() {
   ...
   // add user
	$('.pass_ubd_btn').click(function(){
		$('.pass_ubd').addClass('pop_bl_act');
		$('#html').addClass('ovr_h');
	})
	$('.pass_ubd_back').click(function(){
		$('.pass_ubd').removeClass('pop_bl_act');
		$('#html').removeClass('ovr_h');
	})
	$('.pass_ubd_send').on('click', function(){
		var id = $(this).attr('data-id')
		var pass = $('.pass')
		$.ajax({
			url: "/users/get.php?pass_ubd",
			type: "POST",
			dataType: "html",
			data: ({
				id : id,
				pass : pass.attr('data-val'),
			}),
			success: function(data){
				if (data == 'yes') {
					mess('Пароль сәтті өзгертілді')
					location.reload();
